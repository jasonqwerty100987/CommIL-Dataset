...
AquaSimMac::SendUp(Ptr<Packet> p)
{
  ...
  if (Routing()) {
    m_routingRxTrace(p);
    // std::cout<<"AquaSimMac::HandleIncomingPkt: Mac send up packet to routing layer. "<<std::endl;
  ...
  if (ash.GetDAddr() == AquaSimAddress::ConvertFrom(m_device->GetAddress())) {
    ...
    m_routingRxTrace(p);
    // std::cout<<"AquaSimMac::HandleIncomingPkt: Mac received packet and stopped because no routing layer. "<<std::endl;
    return true;
  }
   m_routingRxTrace(p);
  ...
  if(m_dummyRouting)
  {
      ...
      p->AddHeader(ash);
      // std::cout<<"AquaSimMac::HandleIncomingPkt: Mac re-routing packet. "<<std::endl;
      if (!TxProcess(p)) {
...
AquaSimMac::HandleIncomingPkt(Ptr<Packet> p) {
  ...
  // std::cout<<"AquaSimMac::HandleIncomingPkt: Mac schedule to send up packet to routing layer. Sender address is "<<asHeader.GetSAddr()<<" and the receiver address is "<<asHeader.GetDAddr()<<std::endl;
...
AquaSimMac::Recv(Ptr<Packet> p) {
  ...
  switch (asHeader.GetDirection())
  {
    ...
    case (AquaSimHeader::UP):
      ...
      std::cout<<"AquaSimMac::Recv: Recieved packect at mac layer. Sender address is "<<asHeader.GetSAddr()<<" and the receiver address is "<<asHeader.GetDAddr()<<std::endl;
      HandleIncomingPkt(p);
      return;
  }
