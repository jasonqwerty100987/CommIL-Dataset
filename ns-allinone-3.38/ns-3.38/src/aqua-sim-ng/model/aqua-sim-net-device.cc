...
void 
AquaSimNetDevice::SetSocket(Ptr<Socket> socket_ptr){
  m_socket = socket_ptr;
}
...
AquaSimNetDevice::GetResult(void)
{
  return std::map<int, std::vector<std::string>>(m_result);
}

void
AquaSimNetDevice::UpdateResult(int id, std::string data){
...
void
AquaSimNetDevice::ForwardUp(Ptr<Packet> packet){
  // Ptr<PacketSocket> ps = m_socket->GetObject<PacketSocket>();
  // if(ps){
  //   std::cout << "Succsefully get packetsocket" << std::endl;
  //   AquaSimHeader ash;
  //   packet->PeekHeader(ash);
  //   ps->ForwardUp(this, packet, 0, Address(ash.GetSAddr()), Address(ash.GetDAddr()), NetDevice::PacketType::NS3_PACKET_BROADCAST);
  // }
  AquaSimHeader ash;
  // AquaSimHeader ash1;
  ...
  // packet->PeekHeader(ash);
  // packet->PeekHeader(pstamp);
  // std::cout<< "Received at net device. Sender address is " << Address(ash.GetSAddr()) << "; Destination Address is " << Address(ash.GetDAddr()) << "; The packet size is " << packet->GetSize() << " Bytes;" << std::endl;
  packet->RemoveHeader(ash);
  // packet->RemoveHeader(ash1);
  // packet->RemoveHeader(ash);
  ...
  int payload_size = std::stoi(data_string.substr(data_string.length() - 10, std::string::npos));
  // std::cout<<"The data string is "<<data_string<<", Payload size is "<<payload_size<<"."<<std::endl;
  std::string real_data_string = data_string.substr(data_string.length() - 10 - payload_size - 1, payload_size);
  
  Ptr<Node> this_node = GetNode();
  UpdateResult(this_node->GetId(), real_data_string);
  // std::cout<<"The recieved data is "<<real_data_string<<"; Sender Address is "<<Address(ash.GetSAddr())<<"; Recieving Node Id is "<<this_node->GetId()<<";"<<std::endl;
}

Ptr<Socket>
AquaSimNetDevice::GetSocket(){
  return m_socket;
}
...
AquaSimNetDevice::Send (Ptr< Packet > packet, const Address &dest, uint16_t protocolNumber)
{
  ...
  // std::cout<<"AquaSimNetDevice::Send is called." << std::endl;
...
void
AquaSimNetDevice::SetReceiveCallback (Callback<bool, Ptr<AquaSimNetDevice>, Ptr<const Packet>, uint16_t, const Address&> cb)
{
