...
PktSubmissionTimer::Expire(Ptr<IncomingPacket> inPkt)
{
  ...
  */
  ...
  // std::cout<<"packet sending up from signal cache to sim phy. "<<std::endl;
...
AquaSimSignalCache::AddNewPacket(Ptr<Packet> p){
  ...
  */
  ...
  // std::cout<<"packet received at signal cache. Sender Address is "<<asHeader.GetSAddr()<<"; Receiver address is "<<asHeader.GetDAddr()<<std::endl;
