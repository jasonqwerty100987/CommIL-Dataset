...
AquaSimChannel::Recv(Ptr<Packet> p, Ptr<AquaSimPhy> phy)
{
  ...
  // std::cout<<"packet received in channel layer."<<std::endl;
  return SendUp(p,phy);
}
...
AquaSimChannel::SendUp (Ptr<Packet> p, Ptr<AquaSimPhy> tifp)
{
  ...
  */
  ...
  for (std::vector<PktRecvUnit>::size_type i = 0; i < recvUnits->size(); i++) {
    ...
    // std::cout<<"packet sending to phy layer from channel layer. Sender Address is "<<sender->GetAddress()<<"; Receiver address is "<<recver->GetAddress()<<std::endl;
