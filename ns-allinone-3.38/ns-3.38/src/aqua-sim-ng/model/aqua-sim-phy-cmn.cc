...
AquaSimPhyCmn::Recv(Ptr<Packet> p)
{
  ...
  if (asHeader.GetDirection() == AquaSimHeader::DOWN) {
    ...
    if (p != NULL) {
      ...
      // std::cout<<"packet received at physical layer, sending to signal cache to calculate signal strength. Sender Address is "<<asHeader.GetSAddr()<<"; Receiver address is "<<asHeader.GetDAddr()<<std::endl;
      m_sC->AddNewPacket(p);
    }
  }
  return true;
...
AquaSimPhyCmn::PktTransmit(Ptr<Packet> p, int channelId) {
  ...
  p->PeekHeader(asHeader);
  // std::cout<<"AquaSimPhyCmn::PktTransmit is called." << std::endl;
  if (GetNetDevice()->FailureStatus()) {
    // std::cout<<"AquaSimPhyCmn::PktTransmit node "<<GetNetDevice()->GetNode()->GetId()<<" fails." << std::endl;
    NS_LOG_WARN("AquaSimPhyCmn nodeId=" << GetNetDevice()->GetNode()->GetId() << " fails!\n");
    p = 0;
    return false;
  }
  ...
  {
    // std::cout<<"AquaSimPhyCmn::PktTransmit Unable to reach phy layer (sleep/disable)" << std::endl;
  ...
  case NIDLE:
     ...
     */
    // std::cout<<"AquaSimPhyCmn::PktTransmit something wrong" << std::endl;
    ...
    break;
  case SLEEP:
    // std::cout<<"AquaSimPhyCmn::PktTransmit node " << GetNetDevice()->GetNode()->GetId()<< " is sleeping."<< std::endl;
    NS_LOG_WARN("AquaSimPhyCmn node(" << GetNetDevice()->GetNode()->GetId() << ") is sleeping! (dropping pkt)");
    return false;
    break;
  default:
    // std::cout<<"AquaSimPhyCmn::PktTransmit wrong status (dropping pkt)" << std::endl;
  ...
  */
  ...
  // std::cout<<"AquaSimPhyCmn::PktTransmit sending packet to channel" << std::endl;
  ...
  case MacHeader::UWPTYPE_OTHER:
    if(m_device->MacEnabled()){
      if (!GetMac()->RecvProcess(p)){
        ...
        // std::cout<<"Mac Recv error." << std::endl;
      }else{
        // std::cout<<"received packet send up to mac layer from phy layer." << std::endl;
      }
    }
    break;
  case MacHeader::UWPTYPE_LOC:
    // std::cout<<"received packet send up to mac layer from phy layer through MacLoc." << std::endl;
    GetNetDevice()->GetMacLoc()->Recv(p);
    break;
  case MacHeader::UWPTYPE_SYNC:
    // std::cout<<"received packet send up to mac layer from phy layer through MacSync." << std::endl;
    GetNetDevice()->GetMacSync()->RecvSync(p);
    break;
  case MacHeader::UWPTYPE_SYNC_BEACON:
    // std::cout<<"received packet send up to mac layer from phy layer through MacSyncBeacon." << std::endl;
    GetNetDevice()->GetMacSync()->RecvSyncBeacon(p);
    break;
  case MacHeader::UWPTYPE_NDN:
    // std::cout<<"received packet send up to mac layer from phy layer through NamedData." << std::endl;
    GetNetDevice()->GetNamedData()->Recv(p);
    break;
  default:
    // std::cout<<"AquaSimPhyCmn::SendPktUp unknown error" << std::endl;
...
AquaSimPhyCmn::SignalCacheCallback(Ptr<Packet> p) {
  ...
  // std::cout<<"packet sending up from phy to mac. "<<std::endl;
  SendPktUp(p);
}
