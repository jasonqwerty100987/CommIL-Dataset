...
WifiNetDevice::ForwardUp(Ptr<const Packet> packet, Mac48Address from, Mac48Address to)
{
    ...
    if (type != NetDevice::PACKET_OTHERHOST)
    {
        ...
        // uint32_t packet_size = copy->GetSize();
        // uint8_t* byte_buffer = (uint8_t *)calloc(packet_size, sizeof(uint8_t));
        // uint32_t copied_size = copy->CopyData(byte_buffer, packet_size);
        ...
        // std::cout<<"The data string is "<<data_string<<"."<<std::endl;
        ...
        // UpdateResult(this_node->GetId(), data_string);
    }
    else
    {
        copy->RemoveHeader(llc);
