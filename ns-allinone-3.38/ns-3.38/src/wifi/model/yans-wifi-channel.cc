    ...
    for (PhyList::const_iterator i = m_phyList.begin(); i != m_phyList.end(); i++)
    {
        if (sender != (*i))
        {
            ...
            NS_LOG_DEBUG("propagation: txPower="
                         ...
                         << "m, delay=" << delay);
            // std::cout<<"yans-wifi-channel::send() propagation: txPower="
            //              << txPowerDbm << "dbm, rxPower=" << rxPowerDbm << "dbm, "
            //              << "distance=" << senderMobility->GetDistanceFrom(receiverMobility)
...
YansWifiChannel::GetPropagationLoss(){
    return m_loss;
}
...
YansWifiChannel::Receive(Ptr<YansWifiPhy> phy, Ptr<const WifiPpdu> ppdu, double rxPowerDbm)
{
    ...
    if ((rxPowerDbm + phy->GetRxGain()) < phy->GetRxSensitivity() + RatioToDb(txWidth / 20.0))
    {
        ...
        // std::cout<<"Received signal too weak to process in yans-wifi-channel"<<std::endl;
        // std::cout<<"The rxPowerDbm is "<<rxPowerDbm<<", and the RxGain is "<<phy->GetRxGain()<<", RxSensitivity is "<<phy->GetRxSensitivity()<<", and txWidth is "<<txWidth<<", RatioToDb(txWidth / 20.0) is "<<RatioToDb(txWidth / 20.0)<<"."<<std::endl;
        return;
    }
